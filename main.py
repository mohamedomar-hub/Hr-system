    ...
    try:
        ...
        for index, row in user_salaries.iterrows():
            ...
            with st.expander(f"Salary Details for {month}", expanded=False):
                ...
                st.write(f"**Basic Salary:** {row.get('Basic Salary', 'N/A')}")
                st.write(f"**KPI Bonus:** {row.get('KPI Bonus', 'N/A')}")
                st.write(f"**Deductions:** {row.get('Deductions', 'N/A')}")
                # عرض Net Salary إذا كان العمود موجود
                if "Net Salary" in row.index:
                    st.write(f"**Net Salary:** {row.get('Net Salary', 'N/A')}")
